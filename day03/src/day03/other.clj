...
(def answer
  (let [red-set            (make-paths red [0 0])
        blue-set           (make-paths blue [0 0])
...
(comment
  ...
  ;(def inverted-red (s/map-invert red-blue-intersect))

  (def red-hits
    (for [point red-blue-intersect-vector
          k-and-v (find
                   red-v-inverted point)
          k     (key k-and-v)
          v     (val k-and-v)]
      [k v])))
